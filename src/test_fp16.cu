...
bool test_result_cpu(float *d_C, fp16 *h_A, fp16 *h_B, int M, int N, int K, float relative_tolerance = 0.01f){
    ...
    for(int i = 0; i < M; i++){
        for(int j = 0; j < N; j++){
            for(int k = 0; k < K; k++){
                h_C[i * N + j] += __half2float(h_A[i * K + k]) * __half2float(h_B[j * K + k]);
            }
        }
    }
    // 把 device 拷贝过来对比
...
int main() {
    // A 和 B 都必须为 K-Major
    fp16 *h_A = (fp16 *)malloc(TILE_M * TILE_K * sizeof(fp16));
    for(int i = 0; i < TILE_M * TILE_K; i++){
        h_A[i] = __float2half(i + 1);
    }
    // B 列主序
    ...
    alignas(16) fp16 *d_B;
    float *d_C;
    cudaMalloc(&d_A, TILE_M * TILE_K * sizeof(fp16));
    cudaMalloc(&d_B, TILE_K * TILE_N * sizeof(fp16));
    cudaMalloc(&d_C, TILE_M * TILE_N * sizeof(float));

    // 拷贝数据到 device
    cudaMemcpy(d_A, h_A, TILE_M * TILE_K * sizeof(fp16), cudaMemcpyHostToDevice);
    cudaMemcpy(d_B, h_B, TILE_K * TILE_N * sizeof(fp16), cudaMemcpyHostToDevice);
